...
TOOLS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Используй когда пользователь просит выгрузить данные. "
            "По умолчанию format=excel, если пользователь явно не просит картинку."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "format": {
                    "type": "string",
                    "enum": ["excel", "image"],
                    "description": "Формат: excel (.xlsx) — по умолчанию, или image (PNG) если явно просят картинку",
                    "default": "excel",
                },
                "filename": {
                    "type": "string",
...
Если пользователь прислал готовый SQL — сразу выполняй, без вопросов.

Иначе:
1. УТОЧНЕНИЕ: если запрос неоднозначен — задай один уточняющий вопрос. Не угадывай.
2. ПОКАЗ SQL: если всё понятно — покажи сгенерированный SQL и жди подтверждения (текст или эмодзи).
3. После подтверждения — выполняй.
4. DROP FUNCTION — только после явного текстового "ПОДТВЕРЖДАЮ".

ПРАВИЛА ГЕНЕРАЦИИ SQL:
- Всегда используй SELECT * если пользователь не просил конкретные колонки
...
- Не делай SQL-инъекции, используй параметризованные запросы
...
class ClaudeAgent:
    ...
    def tool_export_data(self, sql: str, format: str = "excel", filename: str | None = None) -> str | bytes:
        clean = sql.strip().upper()
        if not clean.startswith("SELECT") and not clean.startswith("WITH"):
